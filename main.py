...
def create_archive():
    """Create timestamped zip archive of outputs."""
...
def main():
    parser = argparse.ArgumentParser(
        description='Elephant Rumble Denoiser - Remove mechanical noise from bioacoustic recordings'
    )
    parser.add_argument('--csv', required=True, 
                       help='Path to CSV file with call annotations')
    parser.add_argument('--audio', required=True,
                       help='Path to folder containing audio files')
    parser.add_argument('--test', action='store_true',
                       help='Test on first call only (don\'t process full batch)')
    parser.add_argument('--no-archive', action='store_true',
                       help='Skip creating zip archive at the end')
